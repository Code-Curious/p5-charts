function Vaccine(){
  ...
  this.setup = function() {
    ...
    $(selectElement).on('select2:select', function(e) {
      ...
      let firstDosePercentage = parseInt(that.relevantRow.get(1).trim().substring(0, that.relevantRow.get(1).trim().length - 1));
      let remainingPercentageFromFirstDose = 100 - firstDosePercentage;
      let secondDosePercentage = parseInt(that.relevantRow.get(2).trim().substring(0, that.relevantRow.get(2).trim().length - 1));
      let remainingPercentageFromSecondDose = 100 - secondDosePercentage;
      let boosterDosePercentage = parseInt(that.relevantRow.get(3).trim().substring(0, that.relevantRow.get(3).trim().length - 1));
      ...
      for (var i = 0; i < wafflesData.length; i++) {
        let x = 200 + (i * 220);
        that.waffles.push(new Waffle(x, 200, 200, 200, 10, 10, wafflesData[i], waffleNames[i], valuesNames));
      }
    });
  
    // trigger the selection event for the first element loading :
    var data = {
      id: countriesWithIds[0].id,
      text: countriesWithIds[0].text,
    };
